...
```
dig grp%GRP%.%DOMAIN% DNSKEY +noall +answer +multi
```
Should look something like
```
grp%GRP%.%DOMAIN%. 5 IN DNSKEY 257 3 13 (
...
grp%GRP%.%DOMAIN%. 5 IN DNSKEY 256 3 13 (
...
```
dig grp%GRP%.%DOMAIN% DNSKEY +dnssec +noall +answer
```
Should result in something like this
```
grp%GRP%.%DOMAIN%. 5   IN      DNSKEY  257 3 13 PnpghNjNXfy29eidzxizUcAwtdrQ7ylXbcdsmyhSXrW14n5PZYdnZAEu Iubnm6GZoF+p5BX1CuAXwUg+Ela2Eg==
grp%GRP%.%DOMAIN%. 5   IN      DNSKEY  256 3 13 EbjyFdlDwCAYIqZlBiBmvzfQzqag1TM1wIkGDXAdEPoevPmIPe2+4FOD 9AEQCbVUxRvpsosN4rThXTgJk1XPaA==
grp%GRP%.%DOMAIN%. 5   IN      DNSKEY  257 3 13 tZnNamGitG3rL3H3JiA0YCPalEtLYapYvON5xC3ozkfjcSG5c4CsvjB4 /iWeSepoHDXeJxoVWP+F2UULDNEm3A==
grp%GRP%.%DOMAIN%. 5   IN      RRSIG   DNSKEY 13 4 5 20260203193739 20260203182740 3963 grp%GRP%.%DOMAIN%. ASmUp90Pyi53omghejWUcud06P+2CAC4ln7SPgYxOzvKVBzDQ70G1oHl qiWPI1sUn1IbofzerOmXRKCSwPRW0g==
grp%GRP%.%DOMAIN%. 5   IN      RRSIG   DNSKEY 13 4 5 20260203193739 20260203182740 32320 grp%GRP%.%DOMAIN%. 9ykAl8NBXMivhSb4A1ol5Dvjsaq9uZYDUnPEWUYBslHPaYMsgOXzU9Ns mEng3tcvaBl7ZYsDeLD0/GyvWhCdHw==
```
...
```
dig grp%GRP%.%DOMAIN% DS
```

Once the DS record is update we can tell bind that the DS record is updated.
```
sudo rndc dnssec -checkds -key old_id withdrawn grp%GRP%.%DOMAIN%
sudo rndc dnssec -checkds -key new_id published grp%GRP%.%DOMAIN%
```
